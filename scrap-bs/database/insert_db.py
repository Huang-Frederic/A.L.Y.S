...
def insert_genre_if_not_exists(genre):
    ...
    existing_genre_response = (
        supabase.table("genres").select("id").eq("name", genre.name).execute()
    )
    ...
    if existing_genre_response.data:
        ...
        inserted_genre_response = (
            supabase.table("genres").insert({"name": genre.name}).execute()
        )
